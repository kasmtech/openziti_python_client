...
class PostureData(BaseModel):
    ...
    mac: PostureDataMac
    os: PostureDataOs
    processes: List[PostureDataProcess]
    __properties: ClassVar[List[str]] = ["apiSessionPostureData", "domain", "mac", "os", "processes"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.api_session_posture_data:
            for _key_api_session_posture_data in self.api_session_posture_data:
                if self.api_session_posture_data[_key_api_session_posture_data]:
                    _field_dict[_key_api_session_posture_data] = self.api_session_posture_data[_key_api_session_posture_data].to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "apiSessionPostureData": dict(
                (_k, ApiSessionPostureData.from_dict(_v))
                for _k, _v in obj["apiSessionPostureData"].items()
            )
            if obj.get("apiSessionPostureData") is not None
            else None,
            "domain": PostureDataDomain.from_dict(obj["domain"]) if obj.get("domain") is not None else None,
            "mac": PostureDataMac.from_dict(obj["mac"]) if obj.get("mac") is not None else None,
            "os": PostureDataOs.from_dict(obj["os"]) if obj.get("os") is not None else None,
            "processes": [PostureDataProcess.from_dict(_item) for _item in obj["processes"]] if obj.get("processes") is not None else None
        })
        return _obj
