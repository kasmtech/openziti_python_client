...
with openziti_edge_client.ApiClient(configuration) as api_client:
    ...
    api_instance = openziti_edge_client.MFAApi(api_client)
    mfa_auth = openziti_edge_client.MfaCode() # MfaCode | An MFA validation request
...
with openziti_edge_client.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        print("The response of MFAApi->create_totp_token:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling MFAApi->create_totp_token: %s\n" % e)
```
